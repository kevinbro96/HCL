...
class ResNetAdvProp_all(nn.Module):
    def __init__(self, block, num_blocks, pool_len=4, low_dim=128, bn_adv_flag=False, bn_adv_momentum=0.01):
        ...
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1, bn_adv_flag=self.bn_adv_flag,
                                       bn_adv_momentum=self.bn_adv_momentum)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2, bn_adv_flag=self.bn_adv_flag,
                                       bn_adv_momentum=self.bn_adv_momentum)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2, bn_adv_flag=self.bn_adv_flag,
                                       bn_adv_momentum=self.bn_adv_momentum)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2, bn_adv_flag=self.bn_adv_flag,
                                       bn_adv_momentum=self.bn_adv_momentum)
    ...
    def _make_layer(self, block, planes, num_blocks, stride, bn_adv_flag=False, bn_adv_momentum=0.01):
        ...
        for stride in strides:
            layers.append(
                block(self.in_planes, planes, stride, bn_adv_flag=bn_adv_flag, bn_adv_momentum=bn_adv_momentum))
...
class Model(nn.Module):
    def __init__(self, feature_dim=128, bn_adv_flag=False, bn_adv_momentum=0.01):
        super(Model, self).__init__()

        self.f = ResNetAdvProp_all(Bottleneck, [3,4,6,3], low_dim=feature_dim, bn_adv_flag=bn_adv_flag, bn_adv_momentum=bn_adv_momentum)
        # projection head
